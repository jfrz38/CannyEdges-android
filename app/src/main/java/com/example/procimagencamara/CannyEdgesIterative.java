    ...
    private void juntar_contornos() {
        ...
        for(x=0; x < size;x++) {
            ...
            if (matriz_nomax[x] >= u_max){
                ...
                //Comprobar vecinos
                //Izquierda arriba
                if (matriz_nomax[x-width-1] >= u_min) {matriz_umbral[x] = whitePixel; continue;}
                //Arriba
                if (matriz_nomax[x-width] >= u_min) {matriz_umbral[x] = whitePixel; continue;}
                //Derecha arriba
                if (matriz_nomax[x-width+1] >= u_min) {matriz_umbral[x] = whitePixel; continue;}
                //Izquierda
                if (matriz_nomax[x-1] >= u_min) {matriz_umbral[x] = whitePixel; continue;}
                //Derecha
                if (matriz_nomax[x+1] >= u_min) {matriz_umbral[x] = whitePixel; continue;}
                //Izquierda abajo
                if (matriz_nomax[x+width-1] >= u_min) {matriz_umbral[x] = whitePixel; continue;}
